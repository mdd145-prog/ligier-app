...
export async function getProductsByCategory(tipo, opts = {}) {
  ...
  const params = [
    ...
    // Pedimos `extension_attributes.stock_item` para filtrar por stock
    // sin agregar otro filterGroup (5+ filterGroups disparan el WAF de Nexcess).
    `fields=items[sku,price,extension_attributes[stock_item[is_in_stock,qty]]]`,
  ].join('&');
  ...
  try {
    ...
    candidates = (data.items || [])
      .map(p => ({
        ...
        // Magento devuelve is_in_stock como bool y qty como número (0 = sin stock).
        // Si no viene el bloque de stock, NO asumimos en stock — es más seguro descartar.
        inStock: !!(p.extension_attributes?.stock_item?.is_in_stock),
        qty: parseFloat(p.extension_attributes?.stock_item?.qty) || 0,
      }))
  ...
  // Filtro de stock — solo productos efectivamente disponibles para venta.
  ...
  // Random pick + traer detalle completo (con imágenes/atributos) solo de los
  // elegidos. Max 6.
  candidates.sort(() => Math.random() - 0.5);
  const picked = candidates.slice(0, Math.min(count, 6));
  const products = (await Promise.all(picked.map(p => getMagentoProduct(p.sku)))).filter(Boolean);
  return products;
}
