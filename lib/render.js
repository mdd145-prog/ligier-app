...
/**
 * Filtro defensivo: el endpoint custom `api_productos.php?contenido=25` NO
 * está filtrando 750ml estrictamente — se cuelan 375, 1500, etc. (verificado
 * 26 jun 2026: Bienvenida tenía un "Malbec 1500" en el resultado).
 * Hasta que el programador de Magento arregle el filtro, descartamos por name
 * cualquier presentación que NO sea 750ml.
 */
function is750ml(name) {
  if (!name) return false;
  // Presentaciones distintas de 750ml que aparecen en el catálogo Ligier:
  // 187 (mini), 375 (medias), 500, 1000 (1L), 1500 (magnum), 1750, 3000.
  // También variantes en litros ("1lt", "1.5lt").
  if (/\b(187|375|500|1000|1500|1750|3000)\s?(ml|cc)?\b/i.test(name)) return false;
  if (/\b\d+(?:[\.,]\d+)?\s?(?:lt|lts|litros?|liters?)\b/i.test(name)) return false;
  // Estuches/packs (varias unidades de presentación variable):
  if (/\bestuche\s+\d+\b/i.test(name)) return false;
  return true; // por defecto, asumimos 750ml — la mayoría del catálogo
}
...
export async function getProductsByCategory(tipo, opts = {}) {
  ...
  // Filtros: stock real + presentación 750ml (defensivo client-side) +
  // rango de precio + exclude.
  candidates = candidates.filter(p => p.inStock && p.qty > 0);
  candidates = candidates.filter(p => is750ml(p.name));
...
/**
 * Trae las promociones activas hoy desde api_promos.php. Devuelve array de
 * reglas vigentes ({rule_id, name, discount_type, discount_amount, ...}).
 * En caso de error o si la API está caída, devuelve []  (regla:
 * "ante la duda, no mostrar promo" — mejor no comunicar que comunicar
 * algo que no existe).
 */
export async function getActivePromos() {
  try {
    const body = await callCustomApi('/api_promos.php', { activas: 'true' });
    return body.data || [];
  } catch (e) {
    console.error('getActivePromos error:', e.message);
    return [];
  }
}
